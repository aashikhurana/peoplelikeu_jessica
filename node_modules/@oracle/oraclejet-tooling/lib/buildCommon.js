...
module.exports = {
  ...
  buildAllComponents: function _buildAllComponents(context) {
    return new Promise((resolve, reject) => {
      const promises = [];
      // strip down the context parameter.
      const srcBase = path.join(config('paths').src.common, config('paths').src.javascript, config('paths').composites);
      util.log(`buildAllComponents srcBase ${srcBase}`);

      // get path to all debug (non-miniified) components.
      const components = glob.sync('**/!(min)/component.json', { cwd: srcBase });
      Promise.all(promises).then(() => {
        // Return the original context object
        resolve(context);
      }).catch(
      ...
      components.forEach((componentJSONPath) => {
        const componentPath = componentJSONPath.substring(0, componentJSONPath.indexOf('/component.json'));
        util.log(`build component: ${JSON.stringify(componentPath)}`);
        util.copyScriptsToDest(path.join(srcBase, componentPath));
        promises.push(componentPath);
      });
    });
  }
